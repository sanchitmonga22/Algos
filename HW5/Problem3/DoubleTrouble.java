...
public class DoubleTrouble {
    ...
    private static int dfs(Coords currentCoords, char[][] input, boolean[][][][] seen){
        ...
        }else if(!seen[x1+1][y1][x2][y2] && input[x1+1][y1]!='x' && !checkSame(x1+1, y1, x2, y2)){
            a1=dfs(new Coords(x1+1, y1, x2, y2), input, seen);
        }else if(!seen[x1][y1][x2+1][y2] && input[x2+1][y2]!='x' && !checkSame(x1, y1, x2+1, y2)){
        ...
        }else if(!seen[x1][y1+1][x2][y2] && input[x1][y1+1]!='x' && !checkSame(x1, y1+1, x2, y2)){
            a3=dfs(new Coords(x1, y1+1, x2, y2), input, seen);
        }else if(!seen[x1][y1][x2][y2+1] && input[x2][y2+1]!='x' && !checkSame(x1, y1, x2, y2+1)){
    ...
    private static void canExitMaze(char[][] input, Coords iniCoords){
        ...
        int count = dfs(iniCoords, input, seen);
    ...
    public static void main(String[] args) {
		...
		try {
            ...
            Coords initalConfig= new Coords(x1, y1, x2, y2);
